              ...
              <Slider {...setting}>
                {
                  TestimonialData.map((data) => (
                    <div key={data.id} className="my-6 text-white">
                      <div className="flex flex-col gap-4 p-12 shadow-lg mx-4 rounded-xl bg-[#50C878] text-white">
                        <div className="flex justify-start items-center gap-5">
                          <img src={data.img} alt="" className="rounded-full w-16 h-16" />
                          <div>
                            <p className="text-xl font-bold text-white/80 ">{data.name}</p>
                            <p>{data.position}</p>
                          </div>
                        </div>
                        {/* bottom section */}
                        <div className="py-6 space-y-4 flex flex-col items-center gap-4">
                          <p className="text-sm text-white-500">{data.text}</p>
                        </div>
                      </div>
                    </div>
                  ))
